...
#Starting date working with this version: 23_04_2021
#Checked: xxx
...
library(randomForest)
#data_add2$lon <- lon_WRF
#data_add2$lat <- lat_WRF

missing_cells <- which(is.na(data_add2$AOD))
missing_RF <- vector()
rmse_vector <- vector()
r2_vector <- vector()
#Try one
...
for (center_cell in missing_cells) {
  ...
  for (i in c(-4:4)) {
    ...
    coords <- rbind(coords,data_add2[interval[which(interval>0)], colnames(data_add2) %in% c('lon','lat')]) %>% na.omit()
    df <- rbind(df,data_add2[interval[which(interval>0)], colnames(data_add2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                                                     'NDVI','LU_index','T','pop_index','ele','VH_KM','u_wind','v_wind',
                                                                                     'SLP','s_hum','PBL','AOD')]) %>% na.omit()
    
  }
  
  if (length(df$PM25_capa1) > 5) {
    
    model_RF <- randomForest(AOD~., data = df)
    #predictions_RF <- predict(model_RF, df)
    prediction_RF <- predict(model_RF, data_add2[center_cell,])
    rmse <- RMSE(model_RF$predicted, df$AOD)
    rmse_vector <- append(rmse_vector, rmse)
    r2 <- cor(model_RF$predicted, df$AOD)
    r2_vector <- append(r2_vector, r2)
    data_add2[center_cell,]$AOD = prediction_RF
    print(center_cell)
  } else {data_add2[center_cell,]$AOD = NA
...
write.csv(data_add2,"data_add2_square_RF_AOD.csv")
...
sum(is.na(data_add2$AOD))
# 
# data_add_square_RF <- read.csv("data_add_square_RF_AOD.csv")
# 

data_add2 <- read.csv("data_add_square_RF_AOD.csv")


#Hay zonas con tantos NAs que no funcionó lo anterior, hacer segunda pasada con RF tomando el día entero


data_add_2 <- data_add2
...
for (i in c(1:n)) {
  ...
  df <- rbind(data_add_2[lower:upper, colnames(data_add_2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                                  ...
                                                                  'SLP','s_hum','NDVI','PBL','AOD')]) %>% na.omit()             
  if (length(df$PM25_capa1) > 0) {
    ...
    for (k in c(1:length(df$PM25_capa1))) {
      if (is.na(df$AOD[k])) {
        df$AOD[k] = predict(model_RF, newdata = df[k,])
      }
    }  
    data_add_2[lower:upper,]$AOD = df$AOD
    print(i)
...
write.csv(data_add_2,"RF_second_step_AOD_1_2.csv", row.names = FALSE)

sum(is.na(data_add_2$AOD))

RF_second_step <- read.csv("RF_second_step_AOD.csv")

sum(is.na(RF_second_step$AOD))

#1.2.Directly daily RF (acaba con más NAs)
...
sum(is.na(data_add_2$AOD)) #More NAs (70400 than doing the two steps)

RF_second_step <- read.csv("RF_second_step_AOD_1_2.csv")

sum(is.na(RF_second_step$AOD))


#2.full RF (No funciona, demasiada carga computacional)

memory.limit(size = 21000)

XG_first_step <- data_add2[,colnames(data_add2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                          'NDVI','LU_index','T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                          'SLP','s_hum','NDVI','PBL','Month','AOD')] %>% na.omit()

full_RF_model <- randomForest(AOD~., data = XG_first_step)

for (i in c(1:nrow(data_add2))) {
  if (!is.na(data_add2$AOD[i])) {
    data_add2$AOD[i] = data_add2$AOD[i]
  }
  else {
    data_add2$AOD[i] = predict(full_RF_model, newdata = data_add2[,colnames(data_add2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                                                              'NDVI','LU_index','T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                                                              'SLP','s_hum','NDVI','PBL')][i])
  }
}
...
set.seed(2)
index <- sample(c(1:nrow(XG_first_step)), round(0.75*nrow(XG_first_step)))

Y_train = XG_first_step$AOD[index]
X_train <- XG_first_step[index, !colnames(XG_first_step) %in% c('AOD')]
Y_test <- XG_first_step$AOD[-index]
X_test <- XG_first_step[-index, !colnames(XG_first_step) %in% c('AOD')]
...
for (i in c(1:nrow(XG_first_step))) {
  if (!is.na(XG_first_step$AOD[i])) {
    XG_first_step$AOD[i] = XG_first_step$AOD[i]
  }
  else {
    XG_first_step$AOD[i] = predict(xg_model, newdata = as.matrix(XG_first_step[i,-1]))
  }
  print(i)
}

write.csv(XG_first_step, 'XG_first_step_xg.csv', row.names = FALSE)
XG_first_step <- read.csv('XG_first_step_xg.csv')
#LME Model
#para 2-step_RF
data_model2 <- RF_second_step[RF_second_step$D1KMAY_ID %in% ids, ]


#para xgboost
RF_second_step$AOD <- XG_first_step$AOD #sustitute AOD calculated by RF by XGBoost
...
data_model2 <- data_model2[, colnames(data_model2) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
...
#1.Calculate PM2.5 with LMER

data_model_scaled <- scale(data_model2[,!colnames(data_model2) %in% 'yearday'])
...
data_model_scaled$yearday <- data_model2$yearday
...
preds <- predict(Model1, newdata = data_model_scaled, allow.new.levels = TRUE)

preds <-  preds*scale + center

hist(preds)

RF_second_step_lmer <- data_model2

RF_second_step_lmer$PM25 <- preds

#10-fold-cross-validation for 2_RF_step_filled_AOD_LMER

data_model_scaled <- scale(data_model2[,!colnames(data_model2) %in% 'yearday']) %>% data.frame()

data_model_scaled$yearday <- data_model2$yearday
# 
# re_yearday <- unique(as.numeric(row.names(ranef(Model1)$yearday)))
# 
# data_model_scaled <- data_model_scaled[data_model_scaled$yearday %in% re_yearday, ]

data_model_scaled <- na.omit(data_model_scaled)

folds <- createFolds(data_model_scaled$PM25, k = 10)
...
CV <- lapply(folds, function(x) {
  ...
  training_fold = data_model_scaled[-x, ]
  
  test_fold = data_model_scaled[x, ]
  
  Model = lmer(PM25 ~ AOD +T+
  ...
  y_preds = predict(Model, newdata = test_fold, allow.new.levels=T)
...
RF_second_step <- read.csv("RF_second_step_AOD_1_2.csv")
RF_second_step$AOD <- XG_first_step$AOD #sustitute AOD calculated by RF by XGBoost
data_model3 <- RF_second_step[, colnames(RF_second_step) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                                'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
...
data_add2 <- RF_second_step


#2.para xgboost
RF_second_step <- read.csv("RF_second_step_AOD_1_2.csv")
RF_second_step$AOD <- XG_first_step$AOD
...
#continuar
data_add2$yearday <- factor(yday(data_add$fecha))

lon_WRF <- data_add$lon_WRF
lat_WRF <- data_add$lat_WRF

yearday <- data_add2$yearday
D1KMAY_ID <- data_add2$D1KMAY_ID
#Con resuspension

data_add2 <- data_add2[, colnames(data_add2) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                    'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
                                                    ,'PM25_capa2','PM25_resuspension','PM25','D1KMAY_ID')]


#Con resuspension
data_add2=data_add2[,c("AOD","T","SLP","ele","s_hum","u_wind","v_wind","PBL","NDVI","LU_index","pop_index"
                       ,"VH_KM","PM25_capa1","PM25_capa2","PM25_resuspension")]

colnames(data_add2)

data_add_scaled <- scale(data_add2[,!colnames(data_add2) %in% c('D1KMAY_ID','yearday')],
                         scale = fe_scales[-16], center = fe_centers[-16])
...
data_add2$yearday <- yearday
data_add_scaled$yearday <- yearday
data_add2$D1KMAY_ID <- D1KMAY_ID


data_add2$PM25 <- predict(Model1, newdata = data_add_scaled, allow.new.levels = TRUE)*scale + center

hist(na.omit(data_add2$PM25))
colnames(data_add2)



preds <- predict(xg_model, newdata = as.matrix(XG_first_step))

hist(preds)

XG_second_step <- data_model3
XG_second_step$PM25 <- preds
...
RF_second_step$D1KMAY_ID <- data_add$D1KMAY_ID

RF_second_step$PM25 <- data_add2$PM25
...
data_add2$lon_WRF <- lon_WRF
data_add2$lat_WRF <- lat_WRF

data_add2 <- data_add2[data_add2$PM25 < 100, ]
data_add2$PM25 <- ifelse(data_add2$PM25 < 0,0, data_add2$PM25)
...
write.csv(data_year_2015_average, "C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/data_year_2015_average_PM_total_AOD_filled_first.csv")
