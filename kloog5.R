...
library(mice)
data_miced <- mice(data_add)
data_miced <- complete(data_miced)
#fails
...
#Try one
#missing_cells <- 1761
for (number in missing_cells) {
  lower <- number-50
  upper <- number+50
  df <- data_add[c(lower:upper), colnames(data_add) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                           'NDVI','LU_index','PM25')] %>% na.omit()
  model_RF <- randomForest(PM25~., data = df)
  #predictions_RF <- predict(model_RF, df)
  prediction_RF <- predict(model_RF, data_add[number,])
  data_add[number,]$PM25 = prediction_RF
  print(number)
}

#write.csv(data_add,"data_add_simple_RF.csv")
...
#Lets check with only PM2.5 as predictor and LR
...
# rmse_vector <- vector()
# r2_vector <- vector()
# data_add_scaled <- data_add[, colnames(data_add) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
#                                                                                          'NDVI','LU_index','PM25')]
# 
# maxs <- apply(data_add_scaled, 2, max, na.rm = TRUE)
# 
# #Try one
# #missing_cells <- 204
# for (center_cell in missing_cells) {
...
#     
#     coords <- rbind(coords,data_add[interval[which(interval>0)], colnames(data_add) %in% c('lon','lat')]) %>% na.omit()
#     df <- rbind(df,data_add_scaled[interval[which(interval>0)], colnames(data_add_scaled) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
#                                                                                    'NDVI','LU_index','PM25')]) %>% na.omit()
#     
#   }
#   
#   if (length(df_scaled$PM25_capa1) > 5) {
#     
#     model_LR <- lm(PM25~., data = df_scaled)
#     #predictions_RF <- predict(model_RF, df)
#     prediction_LR <- predict(model_LR, newdata = data_add_scaled[center_cell,])
#     unscale(matrix(prediction_LR), matrix(data_add_scaled))
#     rmse <- RMSE(model_LR$fitted.values, df$PM25)
#     rmse_vector <- append(rmse_vector, rmse)
#     r2 <- cor(model_LR$fitted.values, df$PM25)
#     r2_vector <- append(r2_vector, r2)
#     data_add[center_cell,]$PM25 = prediction_LR
#     print(center_cell)
#   } else {data_add[center_cell,]$PM25 = NA
#   missing_LR=append(missing_LR,center_cell)
#   print(paste0(center_cell,'NA'))
#   }
#   
# }
# 
# 
# #write.csv(data_add,"data_add_square_LR.csv")
# 
# write.csv(coords,"df_check_GIS.csv", row.names = FALSE)
# 
# row.names(coords) = NULL
...
# df_check_GIS <- read.csv("df_check_GIS.csv")
# 
# data_add_square_LR <- read.csv("data_add_square_LR.csv")
# 
# 
# data_add <- read.csv("data_add_square_LR.csv")
# 
# mean(r2_vector, na.rm = TRUE)
# 
# mean(rmse_vector[rmse_vector<10], na.rm = TRUE)
# 
# #Hay zonas con tantos NAs que no funcionó lo anterior, hacer segunda pasada con RF tomando el día entero
# 
# data_add_2 <- data_add
...
#     for (k in c(1:length(df$PM25_capa1))) {
#       if (is.na(df$PM25[k])) {
#         df$PM25[k] = predict(model_LR, newdata = df[k,])
#       }
#     }  
#     data_add_2[lower:upper,]$PM25 = df$PM25
#     print(i)
...
# 
# 
# write.csv(data_add_2,"LR_second_step.csv", row.names = FALSE)
# 
# LR_second_step <- read.csv("LR_second_step.csv")
# 
# sum(is.na(LR_second_step$PM25))
# 
...
#Obtain yearly mean by cell with LR calculations in missing values



LR_second_step$lon_WRF <- lon_WRF
LR_second_step$lat_WRF <- lat_WRF

LR_second_step$PM25 <- ifelse(LR_second_step$PM25 < 0,0, LR_second_step$PM25)

#LR_second_step <- na.omit(LR_second_step)

data_year_2015_average <- sqldf('select a.lon_WRF, a.lat_WRF, a.D1KMAY_ID, avg(a.PM25) As PM25, avg(a.PM25_capa1) As capa1, avg(a.PM25_resuspension) As resuspension
                                from LR_second_step As a
                                group by a.D1KMAY_ID')
...
write.csv(data_year_2015_average, "C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/data_year_2015_average_PM_total_square_LR.csv")
