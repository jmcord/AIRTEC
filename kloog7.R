...
#Starting date working with this version: 15_04_2021
#Checked: xxx
...
df_plot <- rbind(data_2015_PM25_esc_aguirre[,c('date','PM25','tipo')],data_2015_PM25_casa_campo[,c('date','PM25','tipo')],
                 data_2015_PM25_caminos[,c('date','PM25','tipo')],data_2015_PM25_castilla[,c('date','PM25','tipo')],
                 data_2015_PM25_malvaro[,c('date','PM25','tipo')],data_2015_PM25_castellana[,c('date','PM25','tipo')],
                 data_2015_PM25_fuenlabrada[,c('date','PM25','tipo')],data_2015_PM25_algete[,c('date','PM25','tipo')])
...
df_plot_agg <- sqldf('select avg(a.PM25) As PM25, a.date
...
#Extract table aggregated by hour for exposure.R

df_plot_agg$hour <- hour(df_plot_agg$date)

df_exposure <- sqldf('select avg(a.PM25) As PM25, a.hour
                     from df_plot_agg As a
                     group by a.hour')

write.csv(df_exposure, 'C:/Users/jcord/Documents/Papers/Paper_exposición_maiac/df_exposure_global.csv', row.names = FALSE)
...
data_add$yearday <- yday(data_add$fecha)

data_add2 <- data_add[data_add$yearday == 251,] %>% na.omit()


write.csv(data_add2,"C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/AOD_251.csv")
...
max_conc_aod <- data_add2[data_add2$fecha == "2015-12-02",]

write.csv(max_conc_day,"C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/max_conc_day.csv")


write.csv(max_conc_aod,"C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/max_conc_aod.csv", row.names = FALSE)
...
#Obtain yearly mean by cell of unprocessed AOD data



AOD_year_2015_average <- sqldf('select a.lon_WRF, a.lat_WRF, a.D1KMAY_ID, avg(a.AOD) As AOD
                                from data_add2 As a
                                group by a.D1KMAY_ID')

sum(is.na(AOD_year_2015_average))

AOD_year_2015_average$AOD <- as.double(AOD_year_2015_average$AOD)
str(AOD_year_2015_average)

write.csv(AOD_year_2015_average, "C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/AOD_year_2015_average.csv", row.names = FALSE)
...
data_model_scaled <- scale(data_model[,!colnames(data_model) %in% 'yearday']) %>% data.frame()
...
#Tengo que hacerlo manual x=folds[1], etc
...
CV <- lapply(folds, function(x) {
  ...
  r2 = RMSE(test_fold$PM25, y_preds)^2
