...
#Starting date working with this version: 20_04_2021
#Checked: xxx
...
#Celdas MAIAC = 1760 (pero ahora s?lo tenemos 1759, faltando la 396) #Celdas WRF = 26880
...
# 
# 
# #write.csv(data, "C:/Users/Jose Mar?a/Documents/AIRTEC/PM/data_model_2015_1_step_apli.csv")
# 
...
df_plot <- rbind(data_2015_PM25_esc_aguirre[,c('date','values','tipo')],data_2015_PM25_casa_campo[,c('date','values','tipo')],
                 data_2015_PM25_caminos[,c('date','values','tipo')],data_2015_PM25_castilla[,c('date','values','tipo')],
                 data_2015_PM25_malvaro[,c('date','values','tipo')],data_2015_PM25_castellana[,c('date','values','tipo')],
                 data_2015_PM25_fuenlabrada[,c('date','values','tipo')],data_2015_PM25_algete[,c('date','values','tipo')])


colnames(df_plot) <- c('date','PM25','tipo')

df_plot_agg <- na.omit(df_plot)
df_plot_agg <- df_plot_agg[,-3]

df_plot_agg <- aggregate(df_plot_agg, by = list(df_plot_agg$date), FUN = mean)
...
colnames(df_plot_agg) <- c('date','PM25','tipo')

df_plot_agg <- df_plot_agg[order(df_plot_agg$date, decreasing = FALSE), ]

dates <- unique(df_plot_agg$date)


df_plot_agg <- aggregate(df_plot[,c('PM25')], by = list('date'=df_plot$date,'tipo'=df_plot$tipo), FUN = 'mean')

colnames(df_plot_agg) <- c('date','tipo','PM25')

#df_plot_agg$date <- dates
...
timeVariation(df_plot, group = 'tipo', pollutant = 'PM25', ylim = c(0,25))

df <- sqldf('select a.tipo, a.date, avg(a.PM25) As PM25
            from df_plot As a
            group by a.tipo,a.date')
...
ids <- unique(data_model$D1KMAY_ID)
...
#Abr_03_2020: Se trata de recuperar lo que se ten?a ahora que no se puede acceder al PC

data_add2 <- read.csv("C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/data_2015_lmer_applied_with_traffic.csv")
data_add <- data_emi
data_add <- data_add[,-c(1:2,7,23:24)]
...
data_add2 <- data_add2[,-c(5,19,21:22,24)]


ggplot(data=data_add2) +
  geom_point(aes(x=AOD, y=PBL))
...
data_full_RF_AOD <- data_add2[,colnames(data_add2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                          'NDVI','LU_index','T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                          'SLP','s_hum','NDVI','PBL','Month','AOD')] %>% na.omit()

set.seed(2)
index <- sample(c(1:nrow(data_full_RF_AOD)), round(0.75*nrow(data_full_RF_AOD)))


data_eru <- data_full_RF_AOD
data_eru_predict <- data_full_RF_AOD[, !colnames(data_full_RF_AOD) %in% c('AOD')]

Y_train = data_full_RF_AOD$AOD[index]
X_train <- data_full_RF_AOD[index, !colnames(data_full_RF_AOD) %in% c('AOD')]
Y_test <- data_full_RF_AOD$AOD[-index]
X_test <- data_full_RF_AOD[-index, !colnames(data_full_RF_AOD) %in% c('AOD')]


set.seed(2)
xg_model <- xgboost(data = as.matrix(X_train), label = Y_train, max.depth = 8,
                    eta = 0.3, nthread = 20, nround = 200, n_estimators = 30,
                    min_child_weight=80,colsample_bytree=0.8,subsample=0.8)

xgb_imp <- xgb.importance(feature_names = xg_model$feature_names,
                          model = xg_model)
xgb.plot.importance(xgb_imp)


preds = predict(xg_model, as.matrix(X_test))

cor(preds, Y_test)^2

XG_first_AOD <- data_add2[,colnames(data_add2) %in% c('PM25_capa1','PM25_capa2','PM25_resuspension',
                                                      'NDVI','LU_index','T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                      'SLP','s_hum','NDVI','PBL','Month','AOD')]

for (i in c(1:nrow(XG_first_AOD))) {
  if (!is.na(XG_first_AOD$AOD[i])) {
    XG_first_AOD$AOD[i] = XG_first_AOD$AOD[i]
  }
  else {
    XG_first_AOD$AOD[i] = predict(xg_model, newdata = as.matrix(XG_first_AOD[i,-1]))
  }
  print(i)
}

write.csv(XG_first_AOD, 'XG_first_AOD_2.csv', row.names = FALSE)
XG_first_AOD <- read.csv('XG_first_AOD_2.csv')

XG_first_AOD$D1KMAY_ID <- data_add2$D1KMAY_ID
XG_first_AOD$yearday <- data_add2$yearday


#######EXPANDED RELATIVE UNCERTAINTY FOR AOD predictions ####

reg <- lm(predict(xg_model, newdata = as.matrix(data_eru_predict))~data_eru$AOD)
summary(reg)

RSS <- sum((predict(xg_model, newdata = as.matrix(data_eru_predict))-reg$coefficients[2]*data_eru$AOD-reg$coefficients[1])^2)


u2cr = function(x)
{
  uncertainty = 2*sqrt(RSS/(length(data_eru$AOD)-2)+0.01^2+
                         (reg$coefficients[1]+(reg$coefficients[2]-1)*x)^2)/predict(xg_model, newdata = as.matrix(data_eru_predict))
}

uncertaintyaod <- u2cr(data_eru$AOD)


plot(data_eru$AOD,uncertaintyaod,cex=0.5,col="red")


xyplot(uncertaintyaod/2~data_eru$AOD,
       type=c('p','smooth'), grid = TRUE, ylim=c(0,1),
       lwd=2, col.line='blue')

plot1aod <- xyplot(uncertaintymlr/2~data_eru$PM25,
...
####END ERU for AOD predictions ####



#K-fold for first XGBoost on AOD gap filling####

data_full_RF_AOD <- na.omit(data_full_RF_AOD)

folds <- createFolds(data_full_RF_AOD$AOD, k = 10)
...
CV <- lapply(folds, function(x) {
  ...
  training_fold = data_full_RF_AOD[-x, ]
  Y_train <- training_fold$AOD
  
  test_fold = data_full_RF_AOD[x, ]
  Y_test <- test_fold$AOD
  
  Model = xgboost(data = as.matrix(training_fold), label = Y_train, max.depth = 8,
                  eta = 0.3, nthread = 20, nround = 200, n_estimators = 30,
                  min_child_weight=80,colsample_bytree=0.8,subsample=0.8)
  
  y_preds = predict(Model, newdata = as.matrix(test_fold))
  
  r2 = cor(test_fold$AOD, y_preds)^2
  
  rmse <- RMSE(test_fold$AOD, y_preds)
  
  mae <- MAE(test_fold$AOD, y_preds)
  
  r2_f <<- c(r2_f,r2)
  
  rmse_f <<- c(rmse_f,rmse)
...
mean(as.numeric(as.character(r2_f)))
sd(as.numeric(r2_f))

mean(as.numeric(as.character(rmse_f)))
sd(as.numeric(rmse_f))

mean(as.numeric(as.character(mae_f)))
sd(as.numeric(mae_f))
...
data_model2 <- XG_first_AOD[XG_first_AOD$D1KMAY_ID %in% ids, ]


data_model2 <- sqldf('select a.*, b.PM25

                        from data_model2 As a

                        left join data_model As b

                        on a.D1KMAY_ID = b.D1KMAY_ID and a.yearday = b.yearday')

#INCISO Prepare data for out-of-AQS-CV####

data_model_CV_AQS <- data_model2
data_model_CV_AQS <- data_model_CV_AQS[, colnames(data_model_CV_AQS) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                                            'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
                                                                            ,'PM25_capa2','PM25_resuspension','Month','AOD','PM25','D1KMAY_ID')] %>% na.omit()



df_model_traffic <- data.frame()
df_CV_traffic <- data.frame()
df_CV_fondo <- data.frame()
df_CV_suburban <- data.frame()
r <- vector()
rmse_cv <- vector()
mae_cv <- vector()
aod_cv <- vector()
for (id in ids) {
  data_model_CV_test <- data_model_CV_AQS[data_model_CV_AQS$D1KMAY_ID == id,]
  data_model_CV_train <- data_model_CV_AQS[data_model_CV_AQS$D1KMAY_ID != id,]
  data_model_CV_test <- data_model_CV_test[,!colnames(data_model_CV_test) %in% 'D1KMAY_ID']
  data_model_CV_train <- data_model_CV_train[,!colnames(data_model_CV_train) %in% 'D1KMAY_ID']
  
  Y_train = data_model_CV_train$PM25
  X_train <- data_model_CV_train[, !colnames(data_model_CV_train) %in% c('PM25')]
  ...
  xgb_imp <- xgb.importance(feature_names = xg_model$feature_names,
                            model = xg_model)
  xgb.plot.importance(xgb_imp)
  n <- which(xgb_imp$Feature=='AOD')
  AOD_imp <- xgb_imp$Importance[n]
  
  preds <- predict(xg_model, newdata = as.matrix(data_model_CV_test[, !colnames(data_model_CV_test) %in% c('PM25')]))
  
  r2 <- cor(preds,data_model_CV_test$PM25)
  r <- c(r,r2)
  mae_cv <- c(mae_cv,MAE(preds,data_model_CV_test$PM25))
  
  rmse_cv <- c(rmse_cv,RMSE(preds,data_model_CV_test$PM25))
  aod_cv <- c(aod_cv,AOD_imp)
  
  #Importante: Si quito casa_de_campo, fuenlabrada y algete, que son suburbana y fondo,
  #los modelos ajustados dan peor r2. Normal porque lo que más tenemos, y juntas, es tráfico
  

}

#Save same type of AQSs for graphication and modstats
df_CV_traffic <- data.frame()
df_CV_traffic <- rbind(df_CV_traffic, data_model_CV_test)
df_CV_traffic$PM25 <- data_model_CV_test$PM25
df_CV_traffic$predicted <- predict(xg_model, newdata = as.matrix(data_model_CV_test[, !colnames(data_model_CV_test) %in% c('PM25')]))
df_model_traffic <- rbind(df_model_traffic, df_CV_traffic)

#Graphication


df_model_traffic <- df_model_traffic[df_model_traffic$PM25 != 1,]

data_plot <- data.frame("observed" = df_model_traffic$PM25,
                        "predicted" = df_model_traffic$predicted,
                        "d" = densCols(df_model_traffic$PM25, df_model_traffic$predicted, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
#FIN INCISO Prepare data for out-of-AQS-CV ####

data_model3 <- data_model2[, colnames(data_model2) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                          'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
                                                          ,'PM25_capa2','PM25_resuspension','Month','PM25')]
...
#Fit model and Calculate PM2.5 with XGBoost

data_model3 <- data_model2[, colnames(data_model2) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                          'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
                                                          ,'PM25_capa2','PM25_resuspension','Month','AOD','PM25')] %>% na.omit()


data_eru <- data_model3
Y_train = data_model3$PM25
X_train <- data_model3[, !colnames(data_model3) %in% c('PM25')]
data_eru_predict <- data_model3[, !colnames(data_model3) %in% c('PM25')]
...
xgb_imp <- xgb.importance(feature_names = xg_model$feature_names,
                          model = xg_model)
xgb.plot.importance(xgb_imp) + xlim(0,30)

data_model3 <- XG_first_AOD[, colnames(XG_first_AOD) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                                'SLP','s_hum','NDVI','PBL','AOD','yearday','PM25_capa1'
                                                                ,'PM25_capa2','PM25_resuspension','Month','AOD')]

data_model3 <- data_model3[, c('AOD','T','SLP','ele','s_hum','u_wind','v_wind','PBL','Month','NDVI','LU_index','pop_index','VH_KM'
                               ,'PM25_capa1'
                               ,'PM25_capa2','PM25_resuspension','yearday')]

preds <- predict(xg_model, newdata = as.matrix(data_model3))

hist(preds)


data_model3$PM25 <- preds

XG_first_AOD_XG <- data_model3

write.csv(XG_first_AOD_XG,'XG_first_AOD_XG.csv',row.names = FALSE)

XG_first_AOD_XG <- read.csv('XG_first_AOD_XG.csv')

#######EXPANDED RELATIVE UNCERTAINTY FOR PM2.5 predictions ####

reg <- lm(predict(xg_model, newdata = as.matrix(data_eru_predict))~data_eru$PM25)
summary(reg)

RSS <- sum((predict(xg_model, newdata = as.matrix(data_eru_predict))-reg$coefficients[2]*data_eru$PM25-reg$coefficients[1])^2)


u2cr = function(x)
{
  uncertainty = 2*sqrt(RSS/(length(data_eru$PM25)-2)-0.15^2+
                         (reg$coefficients[1]+(reg$coefficients[2]-1)*x)^2)/predict(xg_model, newdata = as.matrix(data_eru_predict))
}

uncertaintymlr <- u2cr(data_eru$PM25)


plot(data_eru$PM25,uncertaintymlr,cex=0.5,col="red")

xyplot(uncertaintymlr/2~data_eru$PM25,
       type=c('p','smooth'), grid = TRUE, ylim=c(0,1),
       lwd=2, col.line='blue')

plot1RLM <- xyplot(uncertaintymlr/2~data_eru$PM25,
                   type=c('p','smooth'), grid = TRUE,ylim=c(0,1),
                   lwd=2, col='orange')

plot1aod <- xyplot(uncertaintyaod~data_eru$PM25/50,
                   type=c('p','smooth'), grid = TRUE,ylim=c(0,1),
                   lwd=2, col='blue')
...
####END ERU for PM2.5 predictions ####




#10-fold-cross-validation for XGBoost_filled_AOD_XGBoost


data_model3 <- na.omit(data_model3)

folds <- createFolds(data_model3$PM25, k = 10)
...
CV <- lapply(folds, function(x) {
  ...
  training_fold = data_model3[-x, ]
  Y_train <- training_fold$PM25
  
  test_fold = data_model3[x, ]
  Y_test <- test_fold$PM25
  ...
  rmse <- RMSE(test_fold$PM25, y_preds)
  
  mae <- MAE(test_fold$PM25, y_preds)
...
cor(data_model3$PM25,)


#10-fold-cross-validation for XGBoost_filled_AOD_XGBoost (con data from AQS calculated also)

XG_first_AOD_XG$D1KMAY_ID <- D1KMAY

data_model3 <- XG_first_AOD_XG[XG_first_AOD_XG$D1KMAY_ID %in% ids, ]

data_model3 <- na.omit(data_model3)

y_obs <- data_model$PM25

data_model4 <- sqldf('select a.*,b.PM25 As obs
                     from data_model3 As a
                     left join data_model As b
                     on a.D1KMAY_ID = b.D1KMAY_ID and a.yearday = b.yearday') %>% na.omit()


folds <- createFolds(data_model4$PM25, k = 10)
...
CV <- lapply(folds, function(x) {
  ...
  training_fold = data_model4[-x, ]
  Y_train <- training_fold$PM25
  
  test_fold = data_model4[x, ]
  Y_test <- test_fold$PM25
  ...
  r2 = cor(test_fold$PM25, test_fold$obs)^2
  
  rmse <- RMSE(test_fold$PM25, test_fold$obs)
  
  mae <- MAE(test_fold$PM25, test_fold$obs)
...
cor(data_model4$obs, data_model4$PM25)^2

modStats(data_model4,mod="PM25",obs="obs")

mean(data_model$PM25, na.rm=T)


####ERU for PM2.5 predictions ####
...
#
#RF_second_step <- data_model3         #para 2XG

XG_first_AOD_XG$D1KMAY_ID <- data_add$D1KMAY_ID
...
ea <- data.frame("observed" = dataa$PM25,
                 "predicted" = XG_first_AOD_XG[XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25)
...
data_plot <- data.frame("observed" = datab$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, datab$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = datac$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, dataa$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = datad$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, datad$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = datae$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, datae$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = dataf$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, dataf$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = datak$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, datak$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
data_plot <- data.frame("observed" = datal$PM25,
                        "predicted" =  XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25,
                        "d" = densCols( XG_first_AOD_XG[ XG_first_AOD_XG$D1KMAY_ID == id, ]$PM25, datal$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
#No s? por qu? sale lo mismo que maiac sin RF, revisar, mientras tanto:
...
#Represent predicted vs observed for Stations after 2XG ####

#Function to represent observed vs calculated PM25 and calculate statistics, takes 3 arguments:
  #df: a data frame with the predictions
  #data_add: a data frame with the variable D1KMAY_ID to be extracted
  #df_aqs: the selected AQS that will be calculated
observed_vs_predicted <- function(df, data_add, df_aqs) {
  df$D1KMAY_ID <- data_add$D1KMAY_ID
  id <- df_aqs$D1KMAY_ID[1]
  data_plot <- data.frame("observed" = df_aqs$PM25,
                          "predicted" = df[df$D1KMAY_ID == id, ]$PM25,
                          "d" = densCols(df[df$D1KMAY_ID == id, ]$PM25, df_aqs$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))

  df_mod <- cbind(data_plot$observed, data_plot$predicted) %>% na.omit() %>% data.frame()
  
  print(modStats(df_mod, obs = 'X1', mod = 'X2'))
  
  print(paste0("the average predicted PM25 is: ", mean(df_mod$X2)))
...
observed_vs_predicted(XG_first_AOD_XG, data_add, dataa)

#For type of AQS

observed_vs_predicted <- function(df, data_add, df_aqs) {
  df$D1KMAY_ID <- data_add$D1KMAY_ID
  ids <- unique(df_aqs$D1KMAY_ID)
  
  data_plot <- sqldf('select a.PM25 As observed, b.PM25 As predicted
                     from df_aqs As a
                     left join df As b
                     on a.D1KMAY_ID = b.D1KMAY_ID and a.yearday = b.yearday')
  
  data_plot <- cbind(data_plot,
                          "d" = densCols(df_aqs$PM25, df_aqs$PM25, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
df_traffic <- data_model[data_model$tipo == 'TRAFICO', ]
df_fondo <- data_model[data_model$tipo == 'FONDO', ]
df_sub <- data_model[data_model$tipo == 'SUBURBANA', ]

observed_vs_predicted(XG_first_AOD_XG, data_add, df_traffic)
observed_vs_predicted(XG_first_AOD_XG, data_add, df_fondo)
observed_vs_predicted(XG_first_AOD_XG, data_add, df_sub)
...
#### end Represent predicted vs observed for Stations after 2XG ####


#Obtain yearly mean by cell


XG_first_AOD_XG$lon_WRF <- lon
XG_first_AOD_XG$lat_WRF <- lat
XG_first_AOD_XG$D1KMAY_ID <- data_add2$D1KMAY

XG_first_AOD_XG <- XG_first_AOD_XG[XG_first_AOD_XG$PM25 < 100, ]
XG_first_AOD_XG <- XG_first_AOD_XG[XG_first_AOD_XG$PBL < 5000, ]
XG_first_AOD_XG$PM25 <- ifelse(XG_first_AOD_XG$PM25 < 0,0, XG_first_AOD_XG$PM25)

data_year_2015_average <- sqldf('select a.lon_WRF, a.lat_WRF, a.D1KMAY_ID, avg(a.PM25) As PM25, avg(a.PM25_capa1) As capa1, avg(a.PM25_resuspension) As resuspension, avg(a.PBL) As PBL, avg(a.T) As T, avg(a.s_hum) As s_hum
                                from XG_first_AOD_XG As a
                                group by a.D1KMAY_ID')
...
write.csv(data_year_2015_average, "C:/Users/jcord/Documents/AIRTEC/PM/NetCDF/meteo_2015_average_PM_total_XG_AOD_XG.csv")

#Pool 2XGBoost 


datos_MAIAC_RF_pool <- rbind(ea,campo,cam,cas,mal,cast,fuen,alge)

d = densCols(datos_MAIAC_RF_pool$observed, datos_MAIAC_RF_pool$predicted, colramp = colorRampPalette(rev(rainbow(10, end = 2/6))))

data_plot2 <- data.frame("observed" = datos_MAIAC_RF_pool$observed,
                         "predicted" = datos_MAIAC_RF_pool$predicted,
                         "d" = d)
...
cor(data_plot2$observed, data_plot2$predicted, use = "pairwise.complete.obs")^2

df_mod <- cbind(data_plot2$observed, data_plot2$predicted) %>% na.omit() %>% data.frame()
