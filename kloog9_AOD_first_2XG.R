...
df_plot_agg <- aggregate(df_plot[,c('values')], by = list('date'=df_plot$date,'tipo'=df_plot$tipo), FUN = 'mean')
...
df <- sqldf('select a.tipo, a.date, avg(a.values) As PM25
            from df_plot As a
...
data_model_CV_AQS <- data_model_CV_AQS[, colnames(data_model_CV_AQS) %in% c('T','pop_index','ele','LU_index','VH_KM','u_wind','v_wind',
                                                               ...
                                                               ,'PM25_capa2','PM25_resuspension','Month','PM25','D1KMAY_ID')]
