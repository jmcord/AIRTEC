...
full_RF_model <- randomForest(AOD~., data = data_full_RF_AOD)
...
for (i in c(1:nrow(data_full_RF_AOD))) {
  if (!is.na(data_full_RF_AOD$AOD[i])) {
    data_full_RF_AOD$AOD[i] = data_full_RF_AOD$AOD[i]
  }
  else {
    data_full_RF_AOD$AOD[i] = predict(xg_model, newdata = as.matrix(data_full_RF_AOD[i,-1]))
  }
  print(i)
}

write.csv(data_full_RF_AOD, 'data_full_RF_AOD_xg.csv', row.names = FALSE)
data_full_RF_AOD <- read.csv('data_full_RF_AOD_xg.csv')
#LME Model
#para 2-step_RF
...
#para xgboost
RF_second_step$AOD <- data_full_RF_AOD$AOD #sustitute AOD calculated by RF by XGBoost
...
#2.para xgboost
RF_second_step <- read.csv("RF_second_step_AOD_1_2.csv")
RF_second_step$AOD <- data_full_RF_AOD$AOD
